...
from .utils import prompt_or_clean_directory, repo_dir_name, run_command_with_streaming
...
class SourceConfig:
    ...
    repo: str
    repo_ref: str | None  # None triggers default branch resolution in __post_init__
    workspace_path: str
    ...
    @classmethod
    def from_cli_args(cls, repo: str, repo_ref: str | None, workspace_path: str) -> "SourceConfig":
    ...
    def clone_to_path(self, repo_path: Path, clean: bool = False) -> None:
        ...
        try:
            ...
            returncode = run_command_with_streaming(cmd, logger, stderr_log_func=logger.info)
            ...
            returncode = run_command_with_streaming(cmd, logger, cwd=repo_path, stderr_log_func=logger.info)
...
def clone_workspaces_with_worktrees(
    ...
    os.makedirs(clones_dir, exist_ok=True)  # NOSONAR -- path derived from operator-controlled repo_path, not untrusted input
    ...
    for repo_url, group in groupby(workspaces, key=lambda w: w.source_config.repo):
        ...
        for ws in workspace_list:
            ...
            returncode = run_command_with_streaming(cmd, logger, cwd=clone_path, stderr_log_func=logger.info)
